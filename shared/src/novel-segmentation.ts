const TARGET_READING_UNIT_CHARS = 900;
const MAX_READING_UNIT_CHARS = 1_100;
const V3_TARGET_READING_UNIT_CHARS = 1_800;
const V3_MAX_READING_UNIT_CHARS = 2_000;
...
export function splitNovelText(sourceText: string): string[] {
  const paragraphs = sourceText
    ...
    .filter(Boolean);
  return mergeShortUnits(paragraphs, TARGET_READING_UNIT_CHARS).flatMap((chunk) =>
    splitLongUnit(chunk, MAX_READING_UNIT_CHARS)
  );
}
...
export function splitNovelTextForVersion(sourceText: string, segmentationVersion: number): string[] {
  if (segmentationVersion <= 2) return splitNovelTextLegacy(sourceText);
  if (segmentationVersion === 3) return splitNovelTextV3(sourceText);
  return splitNovelText(sourceText);
}

function splitNovelTextV3(sourceText: string): string[] {
  const paragraphs = sourceText
    .replace(/\r\n?/g, "\n")
    .split(/\n[ \t]*\n+/)
    .flatMap((chunk) => splitBySectionHeadings(chunk))
    .map((chunk) => chunk.trim())
    .filter(Boolean);
  return mergeShortUnits(paragraphs, V3_TARGET_READING_UNIT_CHARS).flatMap((chunk) =>
    splitLongUnit(chunk, V3_MAX_READING_UNIT_CHARS)
  );
}
...
function mergeShortUnits(paragraphs: string[], targetChars: number): string[] {
  const units: string[] = [];
  let current = "";
...
function splitLongUnit(chunk: string, maxChars: number): string[] {
  ...
  for (const paragraph of paragraphs) {
    if (paragraph.length > maxChars) {
      ...
      units.push(...splitLongLine(paragraph, maxChars));
      continue;
    }
