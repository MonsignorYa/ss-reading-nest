...
describe("buildSyncBatches", () => {
  it("keeps complete paragraphs together within the target range", () => {
    ...
    // User/AI-visible batch headers use 「页」, not the old 「段」 unit.
    expect(batches[0]?.text).toContain("【第 1 页】");
    expect(batches[0]?.text).toContain("【第 2 页】");
    expect(batches[1]?.text).toContain("【第 3 页】");
    expect(batches.map((batch) => batch.text).join("")).not.toMatch(/【第 \d+ 段】/);
  });
