...
const bucket = requireEnv("SMOKE_R2_BUCKET");
const databaseId = requireEnv("SMOKE_D1_DATABASE_ID");
...
const client = new Client({ name: "ss-task9-cloud-source-smoke", version: "0.3.19" });
const cleanup = {
  sessions: new Set(),
  r2Keys: new Set()
};
...
try {
  ...
  const bridgeUploadSession = await startSession(`${runId} bridge upload`);
  ...
  const novelSession = await startSession(`${runId} novel`);
  ...
  const keepSourceSession = await startSession(`${runId} keep source`);
  ...
  const deleteCloudSession = await startSession(`${runId} delete cloud`);
...
async function startSession(title) {
  const result = await callTool("start_reading_session", { title });
  const session = result.structuredContent?.session;
  assert(session?.id, "start_reading_session returned no novel id");
